 ...
 * @return {number}
 */
function findNumberOfLISUsingBottomUp(nums: number[]): number {
    ...
    const count: number[] = Array(l).fill(1);
 ...
 * @return {number}
 */
function findNumberOfLISUsingTopDown(nums: number[]): number {
    const l: number = nums.length;
    // dp[i] stores the length of the longest increasing subsequence ending at nums[i].
    const dp: number[] = Array(l).fill(-1);
    // count[i] stores the number of longest increasing subsequences ending at nums[i].
     ...
     * @return {[number, number]} - An array containing [length, count] of LIS ending at i.
     */
